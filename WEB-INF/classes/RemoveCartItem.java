...
public class RemoveCartItem extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        for(OrderItem oi : orderItems){
            if(oi.getName().equalsIgnoreCase(item)){
	...
	protected void displayCart(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(utility.CartCount()>0)
		{
			...
			{
				pw.print("<form name ='removeItem' action='RemoveCartItem' method='post'>");
				pw.print("<tr>");
				pw.print("<td>"+i+".</td><td>"+oi.getName()+"</td><td>: "+oi.getPrice()+"</td>");
				pw.print("<input type='hidden' name='item' value='"+oi.getName()+"'>");
				pw.print("<input type='hidden' name='price' value='"+oi.getPrice()+"'>");
				pw.print("<td><input type='submit' name='rmItem' value='remove'></td></tr></form>");
				...
				pw.print("<tr><td>Please Select Delivery or Pickup:</td><td><select name='orderType'><option value='delivery'>Delivery</option><option value='pickup'>Pickup</option></select></td><td><input type='submit' name='CheckOut' value='CheckOut' class='btnbuy' /></td>");
				pw.print("</table></form>");
