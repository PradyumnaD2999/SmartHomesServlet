...
public class LightingList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
		  ...
		  else if(CategoryName.equals("kasa"))
		  {
			for(Map.Entry<String, Lighting> entry : SaxParserDataStore.lghts.entrySet())
				{
				if(entry.getValue().getRetailer().equals("Kasa"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
		...
		for(Map.Entry<String, Lighting> entry : hm.entrySet()){
			...
			pw.print("<h3>"+lght.getName()+"</h3>");
			pw.print("<strong>"+ "$" + lght.getPrice() + "</strong><ul>");
			pw.print("<li id='item'><img src='images/lght/"+lght.getImage()+"' alt='' /></li>");
			pw.print("<li><form method='post' action='Cart'>" +
					"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
					"<input type='hidden' name='type' value='lghts'>"+
					"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
					"<input type='hidden' name='access' value=''>"+
					"<input type='submit' class='btnbuy' value='Buy Now'></form></li>");
			pw.print("<li><form method='post' action='ViewProduct'>" +
					"<input type='hidden' name='name' value='"+lght.getName()+"'>"+
					"<input type='hidden' name='price' value='"+lght.getPrice()+"'>"+
					"<input type='hidden' name='image' value='"+lght.getImage()+"'>"+
					"<input type='hidden' name='desc' value='"+lght.getDescription()+"'>"+
					"<input type='hidden' name='type' value='lght'>"+
					"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
					"<input type='hidden' name='access' value=''>"+
